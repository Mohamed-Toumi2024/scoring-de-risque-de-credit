...
export function PredictionResult({ result, onReset }: PredictionResultProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Résultat principal avec style amélioré */}
      <Card className="glass border-white/20 shadow-2xl overflow-hidden">
        ...
        <CardContent className="space-y-8 relative z-10">
          {/* Prédiction principale avec animation */}
          <div className="text-center space-y-6 p-6 bg-white/30 rounded-xl backdrop-blur-sm">
            <div className="space-y-3">
              <div
                className={`text-5xl font-bold animate-pulse ${result.prediction === 1 ? "text-red-600" : "text-green-600"}`}
              >
                ...
                className={`text-lg px-6 py-2 shadow-lg ${
                  result.risk_level === "CRITIQUE"
                    ...
                    : result.risk_level === "ÉLEVÉ"
                      ? "bg-gradient-to-r from-orange-500 to-red-500 text-white glow-yellow"
                      : result.risk_level === "MODÉRÉ"
                        ? "bg-gradient-to-r from-yellow-400 to-orange-500 text-white glow-yellow"
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm font-medium">
                ...
                <span
                  className={`font-bold ${result.probability > 0.7 ? "text-red-600" : result.probability > 0.4 ? "text-orange-600" : "text-green-600"}`}
                >
              ...
              <div className="relative">
                <Progress
                  ...
                  className={`h-4 shadow-inner ${
                    result.probability > 0.7 ? "glow-red" : result.probability > 0.4 ? "glow-yellow" : "glow-green"
                  }`}
                />
      ...
      {/* Facteurs d'influence avec style amélioré */}
      <Card className="glass border-white/20 shadow-xl">
        <CardHeader className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-t-lg">
          ...
          <CardDescription className="text-gray-600">
            Indicateurs ayant le plus d'impact sur la prédiction
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6">
          <div className="space-y-4">
           {Array.isArray(result.factors) && result.factors.map((factor, index) => (
              ...
              <div
                key={index}
                className="flex items-center justify-between p-4 rounded-xl bg-white/40 backdrop-blur-sm border border-white/30 hover:bg-white/60 transition-all duration-300 card-hover"
              >
                <div className="flex items-center space-x-4">
                  <div
      ...
      {/* Recommandations avec style amélioré */}
      <Card className="glass border-white/20 shadow-xl">
        <CardHeader className="bg-gradient-to-r from-green-50 to-blue-50 rounded-t-lg">
          <CardTitle className="text-xl text-gray-800">Recommandations</CardTitle>
          <CardDescription className="text-gray-600">
            Actions suggérées pour améliorer la situation financière
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6">
          <div className="space-y-4">
            {Array.isArray(result.recommendations) && result.recommendations.map((recommendation, index) => (

              <div
                key={index}
                className="flex items-start space-x-4 p-4 rounded-xl bg-gradient-to-r from-blue-50/50 to-green-50/50 backdrop-blur-sm border border-white/30 hover:from-blue-100/50 hover:to-green-100/50 transition-all duration-300"
              >
